...
##
ifeq ($(CC.CUSTOM),tcc)
debug: CFLAGS += -UNDEBUG -DSDL_DISABLE_IMMINTRIN_H=1
else
...
##
ifeq ($(CC.CUSTOM),tcc)
release: CFLAGS += -DNDEBUG=1 -DSDL_DISABLE_IMMINTRIN_H=1
else
...
##
ifeq ($(CC.CUSTOM),tcc)
check: CFLAGS += -Werror -Wunsupported -DNDEBUG=1 -DSDL_DISABLE_IMMINTRIN_H=1
else
check: CFLAGS += -Wextra -Werror -Wno-unused-variable -O0 -g3 -DNDEBUG=1
endif
check: CXXFLAGS += -Wextra -Werror -Wno-unused-variable -O0 -g3 -DNDEBUG=1
debug: DFLAGS += --O0 --gc -w
...
ifeq ($(strip $(NO_TES)),)
ifeq ($(CC.CUSTOM),tcc)
cov: CFLAGS += -UNDEBUG -DSDL_DISABLE_IMMINTRIN_H=1 -DTES_BUILD=1
cov: $(TARGETS)
else ifeq ($(strip $(CC.NAME)),gcc)
...
ifeq ($(CC.CUSTOM),tcc)
asan: CFLAGS += -UNDEBUG -DSDL_DISABLE_IMMINTRIN_H=1
asan: $(TARGETS)
else
ifeq ($(strip $(NO_TES)),)
...
## UB-san build
## useful for: squashing undefined behaviour
##
ubsan: REALSTRIP := ':' ; # : is a no-op
ifeq ($(CC.CUSTOM),tcc)
ubsan: CFLAGS += -UNDEBUG -DSDL_DISABLE_IMMINTRIN_H=1
ubsan: $(TARGETS)
else
ubsan: LDFLAGS += -fsanitize=undefined
ifeq ($(strip $(NO_TES)),)
ubsan: CFLAGS += -fno-omit-frame-pointer -O1 -g3 \
	-fno-common -fno-optimize-sibling-calls -fsanitize=undefined \
	-fno-sanitize-recover=all -DTES_BUILD=1
else
ubsan: CFLAGS += -fno-omit-frame-pointer -O1 -g \
	-fno-common -fno-optimize-sibling-calls -UTES_BUILD
endif # NO_TES
endif # CC.CUSTOM
ifeq ($(strip $(NO_TES)),)
ubsan: CXXFLAGS += -fno-omit-frame-pointer -O1 -g3 \
	-fno-common -fno-optimize-sibling-calls -fsanitize=undefined \
	-fno-sanitize-recover=all -DTES_BUILD=1
ubsan: $(TESTARGETS)
else
ubsan: CXXFLAGS += -UTES_BUILD
ubsan: $(TARGETS)
endif
