...
ifeq ($(CC.NAME),clang)
CFLAGS += -ansi -frandom-seed=69420 -x c
else ifeq ($(CC.NAME),gcc)
CFLAGS += -ansi -ffile-prefix-map=OLD=NEW -frandom-seed=69420 -x c
else ifeq ($(CC.CUSTOM),tcc)
CFLAGS += -std=c89 -x c
endif

ifeq ($(CXX.NAME),clang++)
CXXFLAGS += -std=c++11 -frandom-seed=69420 -x c++
else ifeq ($(CXX.NAME),g++)
CXXFLAGS += -std=c++11 -ffile-prefix-map=OLD=NEW -frandom-seed=69420 -x c++
endif
